...
function buyHorse(Modelhor, price, isGold) {        
    ...
    if (isGold) {        
        $.post('https://qr-stable/BuyHorse', JSON.stringify({ ModelH: Modelhor, Gold: price, IsGold: isGold }));
    } else {
        $.post('https://qr-stable/BuyHorse', JSON.stringify({ ModelH: Modelhor, Dollar: price, IsGold: isGold }));    
    }    
}
